...
void RetroChessNotify::notifyChessBusyGxs(const RsGxsId &gxs_id)
{
	emit chessBusyGxs(gxs_id);
}
...
{
	emit chessWatchState(hostId, gameKey, whiteId, whiteName, blackId, blackName, fen, sequence, lastFrom, lastTo, moves);
}
